...
const DuplicateResume = () => {
  ...
  const { duplicate_resume_id } = useRouter().query;
  ...
  useEffect(() => {
    const getResumeById = async () => {
      const response = await fetch(`/api/resume/get/${duplicate_resume_id}`);
      ...
      setEducation(
        data.data?.education?.map((edu: IEducationDetails, index: number) => ({
          id: index + 1,
      ...
      setExperience(
        data.data?.experience?.map(
          (exp: IExperienceDetails, index: number) => ({
            id: index + 1,
            title: exp.title,
            company: exp.company,
      ...
      setProjects(
        data.data?.project?.map((project: IProjectsType, index: number) => ({
          id: index + 1,
          title: project.title,
      ...
      setSkills({
        databases: data.data?.skill?.databases,
        developer_tools: data.data?.skill?.developer_tools,
        frameworks_libraries: data.data?.skill?.frameworks_libraries,
        languages: data.data?.skill?.languages,
      });
      setLoading(false);
    };
  ...
  return (
    <>
      ...
      <Layout currentPage="Build New">
        {loading ? (
          <Center h={"calc(100vh - 100px)"} mt={20}>
